...
exports.run = async (client, message, args, color) => {
  if(!args.length) return args.missing(message, 'no query provided', client.commands.get('manga').help);
  const embed = new RichEmbed();
  const result = await kitsu.searchManga(args.join(' ').replace(/ ,/g, ' '), 0);
  if(!result.length) return message.channel.send('No result found!');
    ...
    embed.setURL(`https://kitsu.io/manga/${atts.slug}`);
		...
		embed.addField("Status", atts.status, true);
